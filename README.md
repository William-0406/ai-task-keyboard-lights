...
状态机决定**显示什么**（"Codex 在干活"），后端决定**怎么在眼前这块硬件上说出来**。
两者分开，是为了让"支持第二块键盘"变成加一个后端，而不是分叉一个项目。

同时存在两套词汇，是有意的：

- `light_backends.EFFECT_SEMANTICS` 把灯效描述成 **(动画, 颜色)**，通用后端需要这个，
  因为它能合成任意颜色。
- 设备配置里的 `packets` 是从厂商驱动抓来的**原始字节**，无法合成——结尾两字节是校验和，
  实测不是任何标准 CRC16——所以抓包后端最忠实，永远优先选用。

语义表不是另起炉灶编的：K500 的包里 `byte[6]` 是动画模式、`byte[9:12]` 是 RGB，
七组抓包数据与语义表**逐项吻合**（`baseline` 的 `00 FF AA` 正是文档里的 RGB(0,255,170)），
并有单元测试锁死这个一致性。

后端按可用性依次选择，`service status` 和 `service.log` 会显示选中了哪个。
**没有任何可用后端时服务会明确报警**，而不是安静地跑着却什么都点不亮。

### 笔记本内置键盘为什么是另一回事

外接 USB 键盘走 HID，能被本项目和 OpenRGB 这类工具触及。**笔记本内置键盘通常不走 HID**
——实测一台 ASUS TUF Gaming A15：内置键盘是 `PS/2 标准键盘 ACPI\MSFT0001`，
机器上一个 ASUS(`0B05`) USB 设备都没有，灯归 `ASUS System Control Interface` 和
Armoury Crate 经 ACPI/WMI 管。每个厂商一套私有接口，因此笔记本键盘是**一个厂商一个集成**
的量级，不是加个后端能覆盖的。
